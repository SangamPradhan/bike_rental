...
class BaseModel extends Model implements HasMedia
{
    ...
    {
        $url = $collectionName
            ? ($this->getFirstMediaUrl($collectionName) ?? null)
            : ($this->getFirstMediaUrl() ?? null);

        // Strip the domain to return only the relative path
        if ($url) {
            $parsedUrl = parse_url($url);
            return $parsedUrl['path'] ?? $url; // Return relative path like /storage/...
        }
